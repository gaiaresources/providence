...
class Db_mysqli extends DbDriverBase {
	/**
	 * @var int The number of times a query should be retried before timing out
	 */
	const MAX_RETRIES = 10;
	protected const MYSQL_ERROR_GONE_AWAY = 2006;
	protected const MYSQL_ERROR_LOST_CONNECTION = 2013;
	...
	public function execute($po_caller, $po_statement, $ps_sql, $pa_values, $pa_options=null) {
		...
		while ($attempt < self::MAX_RETRIES && !$success) {
			...
			if (!($r_res = @mysqli_query($this->opr_db, $vs_sql, caGetOption('resultMode', $pa_options, MYSQLI_STORE_RESULT)))) {
				...
				if ($error_num == self::MYSQL_ERROR_GONE_AWAY || $error_num == self::MYSQL_ERROR_LOST_CONNECTION) { // MySQL server has gone away or Lost connection to MySQL server
					$errorLog = caGetLogger(['logDirectory' => __CA_APP_DIR__.'/log']);
					$errorLog->logWarn("Database connection lost. Trying to reconnect. Attempt $attempt of " . self::MAX_RETRIES);
					// Try to reconnect
					try {
						$this->connect($po_caller, [
							'host' => $this->ops_db_host,
							'username' => $this->ops_db_user,
							'password' => $this->ops_db_pass,
							'database' => $this->ops_db_db,
							'reconnect' => true
						]);
					} catch (DatabaseException $e) {
						$po_statement->postError($this->nativeToDbError($error_num), $error_message, "Db->mysqli->execute()");
						throw new DatabaseException($e->getMessage(), $e->getCode(), "Db->mysqli->execute()");
					}
					// Exponential backoff with a maximum of 60 seconds
					$backoff = min(pow(2, $attempt), 60);
					sleep($backoff);
					continue;
				} else {
					$po_statement->postError($this->nativeToDbError($error_num), $error_message, "Db->mysqli->execute()");
					...
					throw new DatabaseException($error_message, $this->nativeToDbError($error_num), "Db->mysqli->execute()");
				}
			} else {
				if ($attempt > 1) {
					$errorLog = caGetLogger(['logDirectory' => __CA_APP_DIR__.'/log']);
					$errorLog->logInfo("Query successful on retry $attempt of " . self::MAX_RETRIES);
				}
				$success = true;
			}
		}
		if ($attempt === self::MAX_RETRIES) {
			$errorLog = caGetLogger(['logDirectory' => __CA_APP_DIR__.'/log']);
			$errorLog->logError("Gave up retrying query after " . self::MAX_RETRIES . " attempts.");
		}
		if (!$success) {
			throw new DatabaseException($error_message, $this->nativeToDbError($error_num), "Db->mysqli->execute()");
		}
